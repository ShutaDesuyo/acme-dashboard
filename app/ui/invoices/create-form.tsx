'use client'; // 💡 useActionState（フック）を使うため、クライアントコンポーネントにします
...
import { useActionState } from 'react'; // 💡 React最新のエラーハンドリング用フック

export default function Form({ customers }: { customers: CustomerField[] }) {
  const initialState: State = { message: null, errors: {} };
  
  // 💡 useActionStateを使い、アクション（createInvoice）と状態（state）を紐づけます
    ...
    <form action={formAction}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <select
              ...
              defaultValue=""
              aria-describedby="customer-error" // 💡 エラーメッセージの器と紐付け
            >
          ...
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <input
                ...
                className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                aria-describedby="amount-error" // 💡 エラーメッセージの器と紐付け
              />
        ...
        <div aria-live="polite" aria-atomic="true">
          {state.message && (
            <p className="mt-4 text-sm text-red-500 font-semibold">{state.message}</p>
          )}
        </div>
      </div>
